...
public class StudentView {
	...
	public int[] stat(int id) throws SQLException {
	    String str = "SELECT COUNT(CASE WHEN status = 'Present' AND stid = "+id+" THEN 1 END) as present_count FROM attend";
	    String str2 = "SELECT COUNT(CASE WHEN status = 'Absent' AND stid = "+id+" THEN 1 END) as absent_count FROM attend";
	    int[] x = new int[4];
	    Statement stm = con.createStatement();
	    ResultSet rst = stm.executeQuery(str);
	    rst.next();
	    x[1] = rst.getInt("present_count");
	    rst = stm.executeQuery(str2);
	    rst.next();
	    x[2] = rst.getInt("absent_count");
	    x[0] = x[1] + x[2];
	    x[3] = (x[1]*100)/x[0];
	    tblupdt(id);
	    return x;
